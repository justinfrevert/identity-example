...
fn provably_hash(input: &str) -> SessionReceipt {
	let env = ExecutorEnv::builder().add_input(&to_vec(input).unwrap()).build();
...
async fn main() {
	// Parse command line
	let matches = Command::new("hash").arg(Arg::new("message").default_value("")).get_matches();
	...
	api.tx()
		.sign_and_submit_then_watch_default(
			&substrate_node::tx()
				...
				.verify_preimage_proof(substrate_receipt, receipt.journal, HASH_ID),
			&signer,
		)
		.await
		.unwrap()
